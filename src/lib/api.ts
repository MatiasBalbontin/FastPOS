...
export const api = {
  ...
  async deleteProduct(id: string) {
    // Check sales first
    const { count } = await supabase.from('sales').select('*', { count: 'exact', head: true }).eq('product_id', id);
    if (count && count > 0) {
  ...
  async fetchHistory(startDate?: string, endDate?: string) {
    let salesQuery = supabase.from('sales').select('*, products(name)').not('ticket_id', 'is', null);
    let paymentsQuery = supabase.from('customer_payments').select('*, customers(first_name, last_name)');
  ...
  async voidSale(ticket_id: string) {
    const { error } = await supabase.rpc('void_sale', { p_ticket_id: ticket_id });
    if (error) throw error;
  },
  ...
  async fetchAnalytics(startDate: string, endDate: string) {
    const [salesRes, expensesRes, paymentsRes, productsRes] = await Promise.all([
      supabase.from('sales').select('*, products(name, type)').neq('status', 'voided').gte('created_at', `${startDate}T00:00:00`).lte('created_at', `${endDate}T23:59:59`),
      supabase.from('expenses').select('*').gte('created_at', `${startDate}T00:00:00`).lte('created_at', `${endDate}T23:59:59`),
      supabase.from('customer_payments').select('*').neq('status', 'voided').gte('created_at', `${startDate}T00:00:00`).lte('created_at', `${endDate}T23:59:59`),
      supabase.from('products').select('*, batches(quantity, cost)')
    ]);
  ...
  async fetchReceivables() {
    ...
    const [customersRes, salesRes, paymentsRes] = await Promise.all([
      ...
      supabase.from('sales').select('customer_id, quantity, sale_price').eq('payment_method', 'cuenta_por_cobrar').eq('status', 'completed'),
      supabase.from('customer_payments').select('customer_id, amount').eq('status', 'completed')
    ]);
  ...
  async fetchReceivableDetails(customer_id: number) {
    const [customerRes, salesRes, paymentsRes] = await Promise.all([
      ...
      supabase.from('sales').select('ticket_id, created_at, quantity, sale_price, status').eq('customer_id', customer_id).eq('payment_method', 'cuenta_por_cobrar').eq('status', 'completed'),
      supabase.from('customer_payments').select('id, created_at, amount, method, status').eq('customer_id', customer_id)
    ]);
  ...
  async voidPayment(id: string) {
    const { error } = await supabase.from('customer_payments').update({ status: 'voided' }).eq('id', id);
    if (error) throw error;
  }
};
